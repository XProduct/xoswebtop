...
var networkDropSound = null; 
// cache this image   
var airPortError = new Image();
airPortError.src = "resources/Images/AirPortError.png";
...
function pageLoaded() 
{
    ...
    window.addEventListener("online", displayOnlineStatus, true);
    window.addEventListener("offline", displayOnlineStatus, true);
    displayOnlineStatus();
...
{
    monthOnDisplay.setMonth(monthOnDisplay.getMonth() - 12);
    monthOnDisplayUpdated();
}
    ...
    open: function() 
    {
        try {
            if (!window.openDatabase) {
			   //alert("Couldn't open the database.  Please try Google Chrome or Apple Safari.");
			   document.write("No Database Support. Please Try Google Chrome or Apple Safari.");
